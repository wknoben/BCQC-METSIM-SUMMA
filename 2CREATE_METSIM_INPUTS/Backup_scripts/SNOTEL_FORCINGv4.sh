...
for z in range(data_length):												# Iterate through the different snoTEL stations
  ...
  with open ("stationID.csv") as myfile:						# Open a file that contains a list of forcing flie names
    stationID = myfile.read().split('\n')						# Split up original .CSV into rows
	...
	sid = stationID[z]								    # Extract the snoTEL station Id number
	lon = lle['lon'].values[1]							    # Extract the geographical coordinates - latitude
	lat = lle['lat'].values[1]							    # Extract the geographical coordinates - longitude
	elev = lle['elev'].values[1]								      # Extract the geographical elevation
	...
	for i in range(len(Prec)):
		Tmin[i] = Tmin[i]-273.15
		Tmax[i] = Tmax[i]-273.15
		Prec[i] = Prec[i]
