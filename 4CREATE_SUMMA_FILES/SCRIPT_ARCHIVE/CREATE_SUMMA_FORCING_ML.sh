...
with open ("LIST_SWE.csv") as myfile:						# Open list containing names of Metsim-snoTEL simulations
...
########################################
os.chdir('/Users/cjh458/desktop/BCQC-metSIM-SUMMA/6OUTPUT_EVAL/SWE_DATA')						# Change directory into the folder containing swe data
...
for z in range(data_length):											        # Iterate through snotel sites
  ...
  start=0												# declare variable = to the time corresponding to the start of time3 timeseries
  finish=640												# declare variable = to the time corresponding to the end of time3 timeseries
  SWE_DATA = swedata['SWE'].values[start:finish]		# Extract data string as time1 variable
  ...
  xnew=np.linspace(1,640+23/24,num=15360)           # Create a new x variable with the right number of points that correspond to metsim output
...
#######################################
os.chdir('/Users/cjh458/Desktop/GHCND-metSIM-SUMMA-workflow-master/4CREATE_SUMMA_FILES/ERA5_WIND')						# Change directory into the folder containing lists of names
